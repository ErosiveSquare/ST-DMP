...
class Logger(object):
    ...
    def log_accs_table(self, name, accs_list, step, verbose=False):
        ...
        if len(sanitized) == 0:
            return
        accs_array = np.stack(sanitized, axis=0)

        num_rows, num_cols = accs_array.shape
        col_names = [f"task{j}" for j in range(num_cols)]
        index_names = [f"row{i}" for i in range(num_rows)]
        accs_table = pd.DataFrame(accs_array, columns=col_names, index=index_names)
        if verbose:
            print(name)
            print("accs_array shape:", accs_array.shape)
            print(accs_table)
