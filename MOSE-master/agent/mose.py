...
TASK_CKPT_ROOT = os.path.join("cka_data", "ours")
FINAL_CKPT_ROOT = TASK_CKPT_ROOT
FINAL_CKPT_NAME = "ckpt_final.pth"
...
class MOSE(object):
    def __init__(self, model:nn.Module, buffer, optimizer, input_size, args):
        ...
        # 原视图与增强视图一致性约束权重
        print("路径二中main_u>u_u给我换成随机采样，给我删除第二次更新时候合并当前任务流样本，只使用buffer内的样本18.07/22.66，同时feature蒸馏屏蔽ubuffer")
        ...
        self.task_ckpt_root = getattr(args, "task_ckpt_root", TASK_CKPT_ROOT)
        self.final_ckpt_root = getattr(args, "final_ckpt_root", FINAL_CKPT_ROOT)
        self.final_ckpt_name = getattr(args, "final_ckpt_name", FINAL_CKPT_NAME)

        # 取消混合采样策略，Round 1 回退为纯随机采样
    ...
    def train_any_task(self, task_id, train_loader, epoch):
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            for update_round in range(2):
                    ...
                    with autocast():
                        ...
                        if task_id > 0:
                            ...
                            if update_round == 0:
                                ...
                                _ret_rand = self.buffer.sample(buffer_total_batch_size, exclude_task=task_id, ret_ind=True)
                                if len(_ret_rand) >= 4:
                                ...
                                if len(mem_x_rand) > 0:
                                    shared_buffer_x = mem_x_rand.to(x.device, non_blocking=True)
                                    shared_buffer_y = mem_y_rand.to(x.device, non_blocking=True)
                                    main_buffer_indices = main_indices_rand.cpu()
                                else:
                                    # 异常保护（例如缓冲区刚开始为空）
                            ...
                            # -------------------------------------------------------------------------
                                ...
                                # 路径1：U-buffer不确定性更高 → 仅使用U-buffer样本（不合并当前任务流）
                                ...
                                # 路径2：主缓冲区不确定性更高 → 仅使用主缓冲区随机样本（不合并当前任务流）
                                elif main_u > u_u:
                                    _ret_rand = self.buffer.sample(B2, exclude_task=task_id, ret_ind=True)
                                    if len(_ret_rand) >= 4:
                                        mem_x_rand, mem_y_rand, _, main_indices_rand = _ret_rand[0], _ret_rand[1], _ret_rand[2], _ret_rand[3]
                                    else:  # 假设返回3个值
                                        mem_x_rand, mem_y_rand, main_indices_rand = _ret_rand[0], _ret_rand[1], _ret_rand[2]
                                    ...
                                    if len(mem_x_rand) > 0:
                                        ...
                                        # 不再合并当前任务流样本，直接使用主缓冲区样本
                                        cat_x = shared_buffer_x
                                        cat_y = shared_buffer_y
                                        cat_base_size = cat_x.size(0)
                                        cat_u_base_size = 0
                                        cat_view_from_u = False

                                        loss_log['sample/rho'] = 0.0
                                        loss_log['sample/B_sim'] = 0
                                        loss_log['sample/B_rand'] = shared_buffer_x.size(0)
                                    else:
                                        # 主缓冲区无样本可用，跳过第二次更新
                                        continue
                            ...
                            # ts_feat_loss：当前深层 vs 旧模型深层（仅旧类样本，且屏蔽掉来自 U-buffer 的样本）
                            ts_feat_loss = None
                            if self.teacher_student_l2_weight > 0 and old_feat_list is not None:
                                ...
                                # 屏蔽 U-buffer 样本：cat_view_from_u 且 cat_u_base_size 部分在 cat_x 的末尾
                                # 注意 cat_x 做了多视图扩增：cat_input_size = cat_base_size * view_factor
                                if cat_view_from_u and cat_u_base_size > 0 and cat_base_size > 0 and cat_input_size > 0:
                                    # all_y = [new_x (viewed), cat_x (viewed)]
                                    view_factor = cat_input_size // max(cat_base_size, 1)
                                    u_view_count = cat_u_base_size * view_factor
                                    u_view_count = min(u_view_count, cat_input_size)  # 保护

                                    end_cat = new_input_size + cat_input_size
                                    u_start = end_cat - u_view_count

                                    u_mask_full = torch.zeros_like(is_old, dtype=torch.bool)
                                    u_mask_full[u_start:end_cat] = True
                                    # U-buffer 样本统一视为“屏蔽”
                                    is_old = is_old & (~u_mask_full)
                            ...
                            for i in range(len(feat_list)):
                                ...
                                # 知识蒸馏损失：去掉最后一层 logits 的蒸馏
                                distill_loss = 0.
                                if i != last_head_idx and old_pred_list is not None:
                                    old_pred = old_pred_list[i]
                                    
                                    # 创建mask，屏蔽新任务类别
                                    class_mask = torch.ones(pred.size(1), dtype=torch.bool, device=pred.device)
                                    for new_cls in self.new_class_holder:
                                        class_mask[new_cls] = False
                                    
                                    # logits 蒸馏使用全样本
                                    sample_mask = torch.ones(pred.size(0), dtype=torch.bool, device=pred.device)
                                    
                                    if class_mask.any() and sample_mask.any():
                                        pred_old_classes = pred[sample_mask][:, class_mask]
                                        old_pred_old_classes = old_pred[sample_mask][:, class_mask]
                ...
                loss_log['time/elapsed_s'] = time.perf_counter() - start_time
                ...
                if num_d % self.print_num == 0 or batch_idx == 1:
                    ...
                    if task_id > 0:
                        ...
                        print(
                            ...
                            f"{sample_info}, t={loss_log['time/elapsed_s']:.1f}s, {100 * (num_d / self.total_samples):.1f}%"
                        )
                    else:
                        print(
                            f"==>>> it: {batch_idx}, round: {update_round+1}, loss: ins {loss_log['train/ins']:.2f} "
                            ...
                            f"main_u {buf_mean:.3f}, u_u {u_mean:.3f}, t={loss_log['time/elapsed_s']:.1f}s, {100 * (num_d / self.total_samples):.1f}%"
                        )
    ...
    def train(self, task_id, train_loader):
        ...
        # Checkpoint saving disabled per request.
        # task_ckpt_path = self._task_checkpoint_path(task_id)
        # self.save_checkpoint(task_ckpt_path)
        #
        # total_tasks = getattr(self.args, 'n_tasks', task_id + 1)
        # if task_id >= total_tasks - 1:
        #     save_path = getattr(self.args, 'save_path', None) or self._final_checkpoint_path()
        #     self.save_checkpoint(save_path)
        
        total_tasks = getattr(self.args, 'n_tasks', None)
        if total_tasks is None or task_id >= total_tasks - 1:
            self._print_flops_params_info()
    ...
    def _task_checkpoint_path(self, task_id):
        seed = getattr(self.args, 'seed', 0)
        base_dir = self.task_ckpt_root
        if not os.path.isabs(base_dir):
            base_dir = os.path.join(os.getcwd(), base_dir)
        ckpt_root = os.path.join(base_dir, f"run_{seed:02d}")
        os.makedirs(ckpt_root, exist_ok=True)
        return os.path.join(ckpt_root, f"task_{task_id}.pt")

    def _final_checkpoint_path(self):
        seed = getattr(self.args, 'seed', 0)
        base_dir = self.final_ckpt_root
        if not os.path.isabs(base_dir):
            base_dir = os.path.join(os.getcwd(), base_dir)
        ckpt_root = os.path.join(base_dir, f"run_{seed:02d}")
        os.makedirs(ckpt_root, exist_ok=True)
        return os.path.join(ckpt_root, self.final_ckpt_name)

    def _estimate_flops_per_sample(self):
        ...
        dummy = torch.randn(1, *self.input_size, device=device)
        with torch.no_grad():
            try:
                _ = self.model(dummy)
            except Exception:
                try:
                    _ = self.model(dummy, False)
                except Exception:
                    for h in handles:
                        h.remove()
                    if was_training:
    ...
    def _print_flops_params_info(self):
        try:
            n_params = self.model.n_params
        except Exception:
            n_params = sum(p.numel() for p in self.model.parameters())
        flops = self._estimate_flops_per_sample()
        print("=" * 100)
        ...
        print("这里的 FLOPs 指的是 Floating Point Operations（浮点运算次数），用来衡量模型一次前向/训练过程的计算量有多大。")
        print("在你截图的表里：")
        print("#FLOPs：通常表示一次 forward（或一次 iteration / 一个 batch）大概要做多少次浮点运算")
        print("单位写成 M（million，百万），比如 558.55M = 5.5855×10^8 次浮点运算量")
        print("它和 “参数量 #Params” 的区别")
        print("#Params（参数量）：模型里有多少可学习参数，更多代表模型大小/存储开销")
        print("FLOPs（算力开销）：模型跑起来要算多少东西，更多代表速度/计算成本")
        print("同样参数量的模型，FLOPs 也可能差很多（比如多分支、注意力、或更高分辨率特征图）。")
